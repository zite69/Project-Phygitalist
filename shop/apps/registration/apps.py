...
WIZARD_CONDITION_DICT = {
    ...
    "business": True,
    "gst": lambda w: not (hasattr(w.request.user,'seller_registration') or (hasattr(w.request.user,'seller_registration') and (w.request.user.seller_registration.gstin != '' or w.request.user.seller_registration.pan != ''))),
    "pincode": lambda w: not (hasattr(w.request.user,'seller_registration') or (hasattr(w.request.user, 'seller_registration') and w.request.user.seller_registration.pincode == None)),
    "language": lambda w: not (hasattr(w.request.user, 'preferences') or (hasattr(w.request.user, 'preferences') and w.request.user.preferences['second_language'] == '')),
